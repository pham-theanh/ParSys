// this programme will minimize number of iteration
...
public:
	void init3(int nbZ) {
		int p = pow(2, nbZ);
		ConsoX_list = new C_Polyhedron[p];
...
// this function to get order of a variable based on value of the string input
...
// this function is used to compute union of 2 polyhedron which is store in Pointset_Powerset
...
void setSubtract(int * ar1, int * ar2, int *nb1, int nb2) {
	for (int i = 0; i < nb2; i++)
		for (int j = 0; j < *nb1; j++)
			if (ar2[i] == ar1[j])
	...
	if (size > 0) {
		for (int i = 0; i < size; i++) {
...

			{
				...
				cCheck.insert(
						Variable(choseVariable(variNames, numVari, ed.upPara))
								>= 0);
				cCheck.insert(
						Variable(
								choseVariable(variNames, numVari,
										ed.upPara)) <= rate);

			}

			// if upPara is a number will  add it to other linear expression
			else {
			...
			if (choseVariable(variNames, numVari, ed.lowPara) >= 0

			) {

				elow += Variable(choseVariable(variNames, numVari, ed.lowPara));
				l = l - Variable(choseVariable(variNames, numVari, ed.lowPara

				));
				cCheck.insert(
						Variable(choseVariable(variNames, numVari, ed.lowPara
...
void nConsistency(State *s, State states[], Edge * edges, int numState,
	...
	if (s->numChild > 0) {
		...
		for (int i = 1; i < p; i++) {
			...
			intersection1.add_disjunct(s->ConsoX_list[i]);
		...
		inters.intersection_assign(emptyP);
...
// this function is used to compute zero consistency of s
...
void zeroConsistency(State* s, Edge * edges, int numState, string variNames[],
		...
		for (int k = 0; k < s->numZ; k++)
			if (B[k] == 1) {
				...
				if (choseVariable(variNames, numVari, ed.lowPara) >= 0
					...
					l += Variable(
							choseVariable(variNames, numVari, ed.lowPara));
					cs.insert(l == 0);
	...
	C_Polyhedron empty = localConsistency(*s, s->childs, s->numChild, edges,
			numState, variNames, numVari, numTran);

	s->ConsoX_list[0] = empty;
	s->zeroConsistency.add_disjunct(empty);
...
void nConsistency1(State *s, int n, State states[], Edge * edges, int numState,
	...
	if (chkFalse) {
		...
		Pointset_Powerset<C_Polyhedron> trueCheck(numVari + 1); // true value
		Pointset_Powerset<C_Polyhedron> falseCheck(numVari + 1, EMPTY); // false value
		bool chk = false;
		int falseZ[numState], nbfz = 0; // to store states which have value = false are in Z
		int minusX[s->numChild], nem = s->numChild;
		int  flag[s->numZ];
		for (int i =0; i< s->numZ; i++ ) flag[i] = 0;
		...
		for (int i = 0; i < s->numChild; i++)
			if (states[s->childs[i]].consList[n - 1] == falseCheck) {

				int p = getPosition(s->setZ, s->childs[i], s->numZ);
				if (p < 0) {
					// if s has a successor which has nconsistency = false and it is not in Z => nconsistency of s = false

					s->consistencyLevel = s->consistencyLevel + 1;
					chk = true;
					break;
				} else {
					falseZ[nbfz] = s->childs[i]; // store state which has value = false is in Z
					nbfz++;
					flag[i]= 1;
				}
			}
		...
		if (chk == false) {
			...
			else { // has not successor of s which has n-1 consistency = false and it is in Z
...
void zeroConsistency1(State* s, Edge * edges, int numState, string variNames[],
	...
	if (s->consistencyLevel < 0) {
		...
		C_Polyhedron trueCheck(numVari + 1);
		int p = pow(2, s->numZ);
		...
		C_Polyhedron empty = localConsistency(*s, s->childs, s->numChild, edges,
				numState, variNames, numVari, numTran);
		s->ConsoX_list[0] = empty;
		s->consList[0].add_disjunct(empty);
		s->consistencyLevel = 0;
	}
}

void consistency(bool *chk,State * s, int n, State states[], Edge *edges, int numState,
		string variNames[], int numVari, int numTran) {
		...
		*chk = false;
		...
		if (count == s->numChild)
		{
			nConsistency1(s, n, states, edges, numState, variNames, numVari,
					numTran);
		if(s->consList[n]!=s->consList[n-1])	*chk =false;
		}
		else

		{
			for (int i = 0; i < s->numChild; i++)
				consistency(chk,(states + s->childs[i]), n - 1, states, edges,
						numState, variNames, numVari, numTran);
			nConsistency1(s, n, states, edges, numState, variNames, numVari,
					numTran);
			if(s->consList[n]!=s->consList[n-1])*chk =false;
	...
	if (states[0].nConsistency != pCheck)
		...
		while ((k < numState) && (!ok)) {
			...
			for (int i = 0; i < numState; i++) {
				...
				if(states[i].nConsistency != states[i].zeroConsistency)chk=false;
...
string optimAlgorithm(State states[], Edge *edges, string variNames[],
	...
	while ((k < numState) && (!ok)) {
		chk = true;
		consistency(&chk,states + 0, k, states, edges, numState, variNames, numVari,
				numTran);
		cout << "finish with n =" << k << "\n";
...
int main(int argc, char ** args) {
	if (argc < 3)
		...
		if (opt == "1")	myfile1.open("initial_result.txt");
		else myfile1.open("optimal_result.txt");
